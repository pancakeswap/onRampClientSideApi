...
export const getUserTransactions = async (provider: string[], txId?: string) => {
	try {
		...
		if (txId) {
			...
			const transaction = await moonpayTransactionRepository.findOneBy({
...
export const getUserTransactionss = async (req: Request, res: Response, next: NextFunction) => {
	try {
...
export const getAllUserTransactions = async (req: Request, res: Response) => {
	try {
		...
		const transactions: MoonpayTxEntity[] = [];
